...
import { combineLatest, EMPTY, from, merge, Observable, of, ReplaySubject, Subject } from 'rxjs'
import { catchError, map, mergeMap, publishReplay, refCount, startWith, switchMap } from 'rxjs/operators'

import './arrays'
import { databaseUrl, debugPouchDB, remoteDBOnly } from '../config'
...
export function newRemoteDatabase(dbName: string): Database {
  log.debug(`Creating remote database '${dbName}'`)
  const pouchOpts = { skip_setup: true }
  const pouchDB = new PouchDB(databaseUrl + '/' + dbName, pouchOpts)
  const maxLimit = remoteDBOnly ? Number.MAX_SAFE_INTEGER : undefined
  return new Database(pouchDB, log.subLogger('Remote').withPrefix(dbName), maxLimit)
}

export function newLocalDatabase(dbName: string): Database {
  log.debug(`Creating local database '${dbName}'`)
  const pouchOpts = {}
  const pouchDB = new PouchDB(dbName, pouchOpts)
  return new Database(pouchDB, log.subLogger('Local').withPrefix(dbName))
}
...
export class Database {
  ...
  constructor(private db: PouchDB.Database<{}>, log: Logger, private maxLimit?: number | undefined) {
  ...
  private sync: PouchDB.Replication.Sync<{}> | undefined
  private _syncStateListener: SyncStateListener

  private _authenticationCookieTimeout = 600 // In seconds
  private _authenticationCookieRenewal: NodeJS.Timeout | undefined
  ...
  public doSync(other: Database, options?: {}): void {
    ...
    this.sync.on('change', change => {
      ...
      if (this.sync) this.removeCancelable(this.sync)
      this.sync = undefined
    }).on('active', () => {
  ...
  public findOne$<T>(query: PouchDB.Find.FindRequest<T>,
                     ...
                     defaultValue: () => T | null = () => null): Observable<T | null> {
...
export class PouchError extends Error {
  ...
  public constructor(opts: any, methodName?: string) {
    super(
...
export class SyncStateListener {
  ...
  constructor() {
    this.setState(undefined, SyncStatus.COMPLETE)
  }
  ...
  setState(handler: SyncEventsHandler | undefined, status: SyncStatus, error ?: any) {
